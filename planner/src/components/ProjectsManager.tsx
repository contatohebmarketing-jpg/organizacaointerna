    ...
    <div className="flex flex-col gap-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {projects.map((p) => {
          ...
          return (
            <Link key={p.id} href={`/projetos/${p.id}`} className="card p-4 hover:shadow-md transition-shadow">
              ...
              <div className="mt-3 h-1.5 rounded-full bg-cream-200 overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${p.total ? ((p.total - p.open) / p.total) * 100 : 0}%`, backgroundColor: c.hex }} />
              </div>
            </Link>
          );
        })}
        ...
        {!adding ? (
          <button
            ...
            className="rounded-xl border border-dashed border-line text-ink-muted hover:text-teal hover:border-teal p-4 min-h-[110px] flex items-center justify-center text-sm"
          >
            + Novo projeto
          </button>
        ) : (
          <div className="card p-4">
            <input
              ...
              className="w-full rounded-lg border border-line bg-white px-3 py-2 text-sm mb-3"
            />
